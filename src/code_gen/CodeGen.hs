...
generate_stmt (Stmt_Sub_Call name m_raw_expr) =
    value name ++ "(" ++ raw_expr ++ "); " where
        raw_expr = case m_raw_expr of
            Nothing -> ""
            Just e -> value e
generate_stmt (Stmt_Var_Assign name raw_expr) =
    "int " ++ value name ++ " = " ++ value raw_expr ++ "; "
